...
N_z       = 21
baseline  =  model(N_z = N_z)
@unpack β, s, ψ, ρ, σ_ϵ, q, κ, ι, zgrid, P_z  = baseline
idx       = baseline.z_1_idx # z_SS index
dz        = zgrid[2:end] - zgrid[1:end-1]

model_z1(z_1) = model(z_1 = z_1, N_z = N_z, χ = 0, σ_η = 0.273, γ= 0.35, hbar = 1.9547, ε = 0.9424)

# Solve the model for different z_0
@time Threads.@threads for iz = 1:N_z
    modd[iz] =  solveModel(model_z1(zgrid[iz]) , noisy = false)
end
...
p4= plot(zgrid, JJ, label="Hall: Fixed w and fixed a", linecolor=:blue)
