...
@time output     = simulate_moments(Params; check_mult = false)   # skip multiplicity check
...
# Unpack parameters
@unpack std_Δlw, dlw1_du, dlw_dly, u_ss, alp_ρ, alp_σ, u_ss_2, dlu_dlz, std_u, std_z, flag, flag_IR, IR_err  = output
...
@unpack σ_η, χ, γ, hbar, ε, ρ, σ_ϵ = Params
modd  = model(N_z = vary_z_N, χ = χ, γ = γ, hbar = hbar, ε = ε, σ_η = σ_η, ρ = ρ, σ_ϵ = σ_ϵ);
...
yaxis!(L"a(z_1|z_1)");
xlims!((-0.1,0.1));
savefig(p2, file_save*"effort.pdf")
