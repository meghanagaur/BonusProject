...
max_time_nlopt   = 1.5*(60*60)                      # max time for local optimization (in seconds), 0 = no limit
...
@time output = tiktak(init_points, file_save, param_bounds, param_vals, param_est, shocks, data_mom, W, I_max; opt = opt, fix_a = fix_a)
