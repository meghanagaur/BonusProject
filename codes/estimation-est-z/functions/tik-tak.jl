...
function tiktak(init_points, file, param_bounds, param_vals, param_est, shocks, data_mom, W, I_max; 
    I_min  = 10, test = false, max_iters = 50, crit = 1e-6, opt = nothing, fix_a = false)
    ...
    @inbounds for i = 1:N_str
        ...
        if test == false
            ...
            # Local NM-Simplex algorithm with manually enforced bound constraints
            if isnothing(opt)
            
                opt             = Optim.optimize(x -> objFunction_WB(x, start, param_bounds, param_vals, param_est, shocks, data_mom, W; fix_a = fix_a)[1], 
                                    zeros(J), NelderMead(), Optim.Options(g_tol = crit, x_tol = crit,  f_tol = crit, iterations = max_iters, show_trace = true))
