...
"""
Holmstrom & Milgrom (1987) + DMP
Solve static model with unit mass of workers, exponential utility;
optimal linear contract; y = a(z + η), where η ∼ N(0, σ^2).
...
function staticModel(; z = 1, ν = 0.72, ψ = 0.9, κ = 0.213, 
    ...
    β  = 1/(1 + ϕ*r*σ^2)                 # optimal linear contract - slope
    α  = b + ((z*β)^2)*(r*σ^2 - 1/ϕ)/2   # optimal linear contract - level     
    #β = z^2 / (z^2 + ϕ*r*σ^2)           # optimal linear contract - slope
    #α = b + β^2 * (ϕ*r*σ^2 - z^2 )/(2ϕ) # optimal linear contract - level
...
function globalApproxFixedW(x, x0; χ = 0.0)
    ...
    J = y - w           # get expected profits at z = x, holding fixed w at x0
    v = (ψ*J/κ)^(1/ν)   # get vacancies from free entry condition
    n = ψ*v^(1 - ν)     # get job-finding/employment
