...
function rand_shocks(P_z, p_z; z0_idx = 0, N_sim_micro = 10^4, T_sim_micro = 1500, N_sim_macro = 10^4, 
    N_sim_macro_workers = 10^3, T_sim_macro = 828, burnin = 500, N_sim_macro_est_alp = 10^3, set_seed = true, seed = 512)
    ...
    z_idx_micro    = simulateZShocks(P_z, p_z, z_shocks_micro, 1, T_sim_micro + burnin)
...
function simulateWageMoments(η_shocks_micro, s_shocks_micro, jf_shocks_micro, z_idx_micro, N_sim_micro, T_sim_micro, burnin, hp_z, pt_z, f_z, ψ, σ_η, s)
    ...
    act_pb = active[:, burnin+1:end]'
    lw_pb  = lw[:, burnin+1:end]'
    pt_pb  = pt[:, burnin+1:end]'
