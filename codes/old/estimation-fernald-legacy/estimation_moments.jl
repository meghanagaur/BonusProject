...
shocks     = rand_shocks(P_z, p_z; N_sim_micro = N_sim_micro, N_sim_macro = N_sim_macro, N_sim_macro_workers = N_sim_macro_workers, N_sim_macro_est_alp = N_sim_macro_est_alp, z0_idx = z_ss_idx)

if fix_a == false
   
    sol          = solveModel(modd; tol1 = 10^(-13), tol2 = 10^(-13), tol3 =  10^(-13), noisy = false)
    @time output = simulate(modd, shocks; check_mult = false, est_alp = false)        
...
println("W (ss): \t"*string(round.(w_0/modd.ψ, RoundNearestTiesAway, igits=3)))
...
BWF_2    = -(c_term./WF)                # primary BWF measure

println("BWC Share #1: \t\t"*string(round(BWF_2[z_ss_idx], RoundNearestTiesAway, digits = 3)))
println("BWC Share #2: \t\t"*string(round(BWF./WF)[z_ss_idx], RoundNearestTiesAway, digits = 3)))
#println("IWF Share #1: \t\t"*string(round(1 - BWF_2[z_ss_idx], RoundNearestTiesAway, digits = 3)))
#println("IWF Share #2: \t\t"*string(round((IWF./WF)[z_ss_idx], RoundNearestTiesAway, digits = 3)))
#println("WF with χ = 0/WF \t"*string(round((WF_chi0./WF)[z_ss_idx], RoundNearestTiesAway, digits = 3)))
