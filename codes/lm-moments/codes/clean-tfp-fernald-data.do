...
import excel date = A dtfp = N dtfp_unadj = L using "data/fernald-tfp.xlsx", sheet("quarterly") clear
...
foreach var in ltfp ltfp_unadj {
	reghdfe `var', absorb(quarter) resid
	predict `var'_sa, residuals
}
...
pwcorr ltfp_sa_hp ltfp_unadj_sa_hp

* Save the data 
save "data/fernald_tfp", replace 
...
ac ltfp_hp, lags(1) generate(ac_hp)
list ac_hp in 1, clean
pwcorr ltfp_hp l.ltfp_hp

ac ltfp_sa_hp, lags(1) generate(ac_sa_hp)
list ac_sa_hp  in 1, clean
pwcorr ltfp_sa_hp l.ltfp_sa_hp

* Compute autocorrelations for lambda = 1600
ac ltfp_hp1600, lags(1) generate(ac_hp1600)
list ac_hp1600 in 1, clean
pwcorr ltfp_hp1600 l.ltfp_hp1600

ac ltfp_sa_hp1600, lags(1) generate(ac_sa_hp1600)
list ac_sa_hp1600 in 1, clean
