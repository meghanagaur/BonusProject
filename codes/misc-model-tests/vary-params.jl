...
function partial(q_0; modd = model(), max_iter2 = 1000, tol2 = 10^-8,  max_iter3 = 1000, tol3 = 10^-8)
    ...
    wage_flag   = (w_0 <= 0)
    effort_flag = maximum(a_flag)

    # Check IR constraint (must bind)
    return (Y = Y_0[z_1_idx], V = U, ω_0 = ω_0, w_0 = w_0,
         a = az[z_1_idx], flag = max(wage_flag, effort_flag),
         a_min = az[1], a_max = az[end])
...
a_min = [modd[i].a_min for i = 1:length(qgrid)]
a_max = [modd[i].a_max for i = 1:length(qgrid)]
flag  = [modd[i].flag for i = 1:length(qgrid)]
...
p4 = plot(qgrid, flag , ylabel=L"flag", xlabel=L"q", legend=false)

plot(p1, p2, p3, p4,  layout = (2, 2))
savefig(dir*"figs/vary_q_eps.png")

# Plot the different effort levels
plot(qgrid, a , label=L"a(z_1)", xlabel=L"q")
plot!(qgrid, a_min , label=L"a(\underbar{z})", xlabel=L"q")
plot!(qgrid, a_max , label=L"a(\bar{z})", xlabel=L"q", legend=:right)

savefig(dir*"figs/a_vary_q.png")
