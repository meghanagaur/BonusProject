...
p1 = binscatter(df, @formula(y ~ x), 50, xlabel=L"\varepsilon")
df = DataFrame(y = fvals, x = σ_η_vals )
p2 = binscatter(df, @formula(y ~ x), 50, xlabel=L"\sigma_\eta")
df = DataFrame(y = fvals, x = χ_vals)
p3 = binscatter(df, @formula(y ~ x), 50, xlabel=L"\chi")
df = DataFrame(y = fvals, x = χ_vals)
p4 = binscatter(df, @formula(y ~ x), 50, xlabel=L"\gamma")
