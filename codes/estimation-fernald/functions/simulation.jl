        ...
        if est_alp == true
            ...
            Threads.@threads for n = 1:N_sim_macro_alp
                alp_ρ_n[n], alp_σ_n[n], dlu_dly_n[n] = simulateALP(z_idx_macro[:,n], s_shocks_macro, jf_shocks_macro, N_sim_macro_alp_workers, 
                                                                    T_sim_macro, burnin, T_q_macro, s, f_z, y_z; λ = λ)
            end
...
function rand_shocks(P_z, p_z; z0_idx = 0, N_sim_micro = 2*10^4, T_sim_micro = 1000, 
    ...
    s_shocks_macro  = rand(Uniform(0,1), N_sim_macro_alp_workers, T_sim_macro + burnin)  # separation shocks: N x T
    jf_shocks_macro = rand(Uniform(0,1), N_sim_macro_alp_workers, T_sim_macro + burnin)  # job-finding shocks: N x T
    ...
    return (z_idx_micro = z_idx_micro, z_idx_macro = z_idx_macro, N_sim_micro = N_sim_micro, T_sim_micro = T_sim_micro, 
            ...
            N_sim_macro = N_sim_macro, N_sim_macro_alp_workers = N_sim_macro_alp_workers, T_sim_macro = T_sim_macro, burnin = burnin, 
            s_shocks_macro = s_shocks_macro, jf_shocks_macro = jf_shocks_macro, N_sim_macro_alp = N_sim_macro_alp)
end
